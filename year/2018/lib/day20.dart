...
String findGroup(String line, int start) {
  if (line[start] == '(') {
    var openBrackets = 0;

    for (var i = start + 1; i < line.length; i++) {
      final currentLetter = line[i];
    ...
    throw Exception('Wrong start!');
  }
}

Iterable<String> splitGroup(String group) sync* {
  var start = 0;
  var openBrackets = 0;

  for (var i = 0; i < group.length; i++) {
    final currentLetter = group[i];
    ...
    if (currentLetter == '(') {
      ...
      if (openBrackets == 0) {
        yield group.substring(start, i);
        start = i + 1;
      }
    }
  }

  yield group.substring(start);
}

Iterable<String> parse(String line) sync* {
  var end = false;
  ...
  for (var i = 0; i < line.length; i++) {
    ...
    if (currentLetter == '^') {
      ...
      sb.write(currentLetter);
    } else {
      final group = findGroup(line, i);
      i += group.length + 2;

      for (final part in splitGroup(group)) {
        yield* parse('$sb$part${line.substring(i)}');
      }
