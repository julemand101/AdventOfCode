...
List<String> getPrograms(int length) => List.generate(
  length,
  (x) => String.fromCharCode(codeUnitForLetterSmallA + x),
  growable: false,
);

String solveA({required int programCount, required String danceMoves}) {
  final programs = getPrograms(programCount);
  _parse(danceMoves.split(',')).forEach((command) => command.execute(programs));
  return programs.join();
}

String solveB({
  required int programCount,
  required String danceMoves,
  required int times,
}) {
  final programs = getPrograms(programCount);
  final commands = _parse(danceMoves.split(','));
